...
async function init() {
  try {
    ...
    // 获取早安心语
    const zaoan = await fetch(
      `http://api.tianapi.com/zaoan/index?key=${tianXingKey}`
    );
    const zaoanData = await zaoan.json();
    const { content } = zaoanData.newslist[0];

    //获取星座运势
    const xingzuo = await fetch(
      `http://api.tianapi.com/star/index?key=${tianXingKey}&astro=${encodeURI('摩羯座')}`
    );
    const xingzuoData = await xingzuo.json();
    const  xingzuoyunshi  = xingzuoData.newslist;
    ...
    // 用邮件模版生成字符串
    const htmlStr = emailHtml(weatherData, lifeData, word, imgurl, lovingDays,content,xingzuoyunshi);
